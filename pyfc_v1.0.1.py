...
"""
================================================================
🚀 PyFlowCode — 节点式 Python 代码可视化编辑器 (新手友好升华版)
================================================================
...
def register_builtin_nodes():
    ...
    register_node(_make_binary_op("op_sub", "减法 (a - b)", "-", lambda a, b: a - b))
    register_node(_make_binary_op("op_mul", "乘法 (a * b)", "*",
        lambda a, b: (a * b) if not (isinstance(a, str) and isinstance(b, int)) else a * b))
    register_node(_make_binary_op("op_div", "除法 (a / b)", "/", lambda a, b: a / b, "float"))
    register_node(_make_binary_op("op_floordiv", "整除 (a // b)", "//", lambda a, b: a // b))
    register_node(_make_binary_op("op_mod", "取余 (a % b)", "%", lambda a, b: a % b))
    register_node(_make_binary_op("op_pow", "幂 (a ** b)", "**", lambda a, b: a ** b))
    register_node({
        "type": "op_neg", "label": "负号 (-a)", "category": "operator",
    ...
    # --- 逻辑 ---
    register_node({
        "type": "logic_and", "label": "与 (a and b)", "category": "logic",
    ...
    register_node({
        "type": "logic_or", "label": "或 (a or b)", "category": "logic",
    ...
    register_node({
        "type": "logic_not", "label": "非 (not a)", "category": "logic",
    ...
    # --- 容器 ---
    register_node({
        "type": "list_make", "label": "构造列表", "category": "container",
        "inputs": [{"id": "item0", "label": "元素 0", "type": "any"}, {"id": "item1", "label": "元素 1", "type": "any"}, {"id": "item2", "label": "元素 2", "type": "any"}],
    ...
    register_node({
        "type": "list_length", "label": "长度 (len)", "category": "container",
    ...
    register_node({
        "type": "list_index", "label": "索引 (obj[i])", "category": "container",
        "inputs": [{"id": "obj", "label": "对象", "type": "any", "default": []}, {"id": "index", "label": "索引", "type": "int", "default": 0}],
    ...
    register_node({
        "type": "list_append", "label": "追加 (append)", "category": "container",
        "inputs": [{"id": "list", "label": "列表", "type": "list", "default": []}, {"id": "item", "label": "元素", "type": "any"}],
    ...
    # --- 字符串 ---
    register_node({
        "type": "str_format", "label": "字符串格式化 (f-string)", "category": "string",
        "inputs": [{"id": "template", "label": "模板", "type": "str", "default": "Hello, {0}!", "editable": True}, {"id": "arg0", "label": "参数 0", "type": "any"}, {"id": "arg1", "label": "参数 1", "type": "any"}],
    ...
    register_node({
        "type": "str_concat", "label": "字符串拼接", "category": "string",
    ...
    # --- 控制流 ---
    register_node({
        "type": "control_if", "label": "条件选择 (If)", "category": "control", "isControlFlow": True,
        "inputs": [{"id": "condition", "label": "条件", "type": "bool", "default": True}, {"id": "true_value", "label": "真值", "type": "any"}, {"id": "false_value", "label": "假值", "type": "any"}],
    ...
    register_node({
        "type": "control_for_each", "label": "遍历 (For Each)", "category": "control", "isControlFlow": True,
        "inputs": [{"id": "iterable", "label": "可迭代", "type": "any", "default": []}],
        "outputs": [{"id": "items", "label": "元素列表", "type": "list"}, {"id": "count", "label": "数量", "type": "int"}],
    ...
    register_node({
        "type": "control_filter", "label": "过滤 (Filter)", "category": "control", "isControlFlow": True,
        "inputs": [{"id": "iterable", "label": "列表", "type": "list", "default": []}, {"id": "func", "label": "过滤函数", "type": "callable"}],
        "outputs": [{"id": "result", "label": "结果", "type": "list"}], "execute": None,
        "to_code": lambda ctx, inp: {"assignments": {"result": f"list(filter({inp['func']}, {inp['iterable']}))"}},
    })
    register_node({
        "type": "control_map", "label": "映射 (Map)", "category": "control", "isControlFlow": True,
        "inputs": [{"id": "iterable", "label": "列表", "type": "list", "default": []}, {"id": "func", "label": "映射函数", "type": "callable"}],
        "outputs": [{"id": "result", "label": "结果", "type": "list"}], "execute": None,
    ...
    # --- 函数 ---
    register_node({
        "type": "func_define", "label": "定义函数 (def)", "category": "function", "isSubgraph": True,
        "inputs": [{"id": "param0", "label": "参数 0", "type": "any"}, {"id": "param1", "label": "参数 1", "type": "any"}],
        "outputs": [{"id": "func", "label": "函数", "type": "callable"}], "execute": None, "to_code": None,
    })
    register_node({
        "type": "func_call", "label": "调用函数 (call)", "category": "function",
        "inputs": [{"id": "func", "label": "函数", "type": "callable"}, {"id": "arg0", "label": "参数 0", "type": "any"}, {"id": "arg1", "label": "参数 1", "type": "any"}],
        "outputs": [{"id": "return", "label": "返回值", "type": "any"}], "execute": None,
        "to_code": lambda ctx, inp: {"assignments": {"return": f"{inp['func']}({', '.join(inp.get(k,'') for k in ['arg0','arg1'] if inp.get(k))})"}},
    })
    register_node({
        "type": "func_return", "label": "返回值 (return)", "category": "function",
        "inputs": [{"id": "value", "label": "值", "type": "any"}], "outputs": [],
...
class Interpreter:
    ...
    def run(self):
        ...
        for node in sorted_nodes:
            ...
            if node["type"] == "note": continue # 忽略注释节点
            try:
    ...
    def _execute_node(self, node, nodes, edges, depth, call_stack):
        defn = get_node_def(node["type"])
        if not defn: raise Exception(f"未知节点类型: {node['type']}")
    ...
    def _execute_call(self, node, inputs, ctx, depth, call_stack):
        ...
        if not isinstance(func, PyClosure): raise Exception("Call 节点的 func 输入不是可调用对象")
        if depth >= self.max_depth: raise Exception(f"达到最大调用深度 {self.max_depth}")
        if node["id"] in call_stack: raise Exception(f"检测到递归调用循环")
    ...
    def _execute_filter_map(self, node, inputs, ctx, depth, call_stack, is_map):
        ...
        for i, item in enumerate(arr):
            if i >= self.max_iterations: raise Exception("达到最大迭代次数")
...
def generate_python(nodes, edges, header=True):
    ...
    lines = []
    if header:
        lines.extend(["#!/usr/bin/env python3", "# -*- coding: utf-8 -*-", '"""', "由 PyFlowCode 节点式编辑器生成", '"""', ""])
    if func_defs:
...
EXAMPLES = {
    "hello_world": {
        "name": "Hello World", "description": "最简单的字符串打印", "format_version": 1,
        "nodes": [
    ...
    "arithmetic": {
        "name": "算术运算", "description": "5 + 3 * 2 的运算", "format_version": 1,
        "nodes": [
...
class NodeItem(QGraphicsItem):
    ...
    def paint(self, painter, option, widget):
        ...
        if self.node["type"] == "note":
            ...
            text = self.node.get("params", {}).get("text", "双击编辑注释...")
        ...
        if defn.get("isSubgraph"):
            painter.setPen(QPen(QColor("#fbbf24")))
            painter.setFont(QFont("", 7))
            painter.drawText(QRectF(6, self.height - 18, self.width - 12, 14), Qt.AlignVCenter | Qt.AlignLeft, "双击进入子图 →")
    ...
    def _port_at(self, local_pos):
        for pid, info in {**self.input_ports, **self.output_ports}.items():
            ...
            dx = local_pos.x() - p.x()
            dy = local_pos.y() - p.y()
            if dx * dx + dy * dy <= (PORT_RADIUS + 12) ** 2: # 磁吸范围扩大
    ...
    def mouseDoubleClickEvent(self, event):
        ...
        if defn and defn.get("isSubgraph"):
            ...
            return
        if self.node["type"] == "note":
            text, ok = QInputDialog.getMultiLineText(self.scene().views()[0], "编辑注释", "请输入注释内容:", self.node.get("params", {}).get("text", ""))
...
class CanvasScene(QGraphicsScene):
    ...
    def finish_connection(self, from_node, from_port, to_node, to_port):
        ...
        if not is_type_compatible(from_type, to_type):
            self.main_window.console.append(f"❌ 连线失败: 类型不兼容 (<b>{from_type}</b> ➔ <b>{to_type}</b>)", "error")
            return
...
class QuickSearchDialog(QDialog):
    def __init__(self, main_window, pos):
        ...
        self.search_edit.setPlaceholderText("🔍 搜索节点... (回车添加)")
...
# ============================================================================
# Qt UI: 其他面板 (Palette, Properties, Console, Dialog)
# ============================================================================
...
class PropertiesPanel(QWidget):
    ...
    def show_node(self, node_id):
        ...
        if node["type"] == "note":
            self.content_layout.addWidget(self._section_label("📝 注释内容 (双击画布节点编辑)"))
...
class MainWindow(QMainWindow):
    def __init__(self):
        super().__init__()
        self.setWindowTitle("PyFlowCode — 节点式 Python 代码可视化编辑器 (升华版)")
        ...
        self.console.append("💡 <b>提示 1</b>: 在画布空白处 <b>右键</b> 或按 <b>Tab</b> 键可快速搜索并添加节点", "info")
        self.console.append("💡 <b>提示 2</b>: 连线时请确保 <b>端口类型颜色</b> 匹配 (如 蓝色 int ➔ 蓝色 int)", "info")
        self.console.append("💡 <b>提示 3</b>: 节点太乱？点击顶部工具栏的 <b>✨ 整理画布</b> 一键排版", "info")
    ...
    def _build_toolbar(self):
        ...
        toolbar.addAction(QAction("▶ 运行", self, triggered=self.run_workflow))
        toolbar.addAction(QAction("</> 生成 Python", self, triggered=self.generate_code))
        toolbar.addAction(QAction("✨ 整理画布", self, triggered=self.auto_layout)) # 新增功能
        toolbar.addSeparator()
        toolbar.addAction(QAction("💾 导出", self, triggered=self.export_document))
        toolbar.addAction(QAction("📂 导入", self, triggered=self.import_document))
        toolbar.addSeparator()
        toolbar.addAction(QAction("📚 示例", self, triggered=self._show_examples_menu))
        toolbar.addAction(QAction("🗑 清空", self, triggered=self.clear_canvas))
    ...
    def _build_statusbar(self):
        ...
        self.status_label = QLabel("就绪 | 快捷键: Tab (搜索), Del (删除), Ctrl+S (保存)")
    ...
    def auto_layout(self):
        ...
        self.reload_canvas()
        self.console.append("✨ 画布已自动整理排版", "info")

    def run_workflow(self):
        if self.subgraph_path: self.console.append("请在顶层画布运行工作流", "error"); return
    ...
    def import_document(self):
        ...
        if path:
            try:
                ...
                if "nodes" not in doc or "edges" not in doc: raise Exception("无效的 .pyflow 文件")
                self.load_document(doc)
                self.console.append(f"✓ 已加载工作流 ({len(doc['nodes'])} 节点, {len(doc['edges'])} 连线)", "info")
            except Exception as e: self.console.append(f"导入失败: {e}", "error")
    ...
    def _load_example(self, key):
        ...
        self.console.append(f"✓ 已加载示例: {EXAMPLES[key]['name']}", "info")
    ...
    def _check_autosave(self):
        ...
        if os.path.exists(save_path):
            reply = QMessageBox.question(self, "恢复草稿", "检测到上次未完成的编辑，是否恢复草稿？", QMessageBox.Yes | QMessageBox.No)
            if reply == QMessageBox.Yes:
                try:
                    with open(save_path, "r", encoding="utf-8") as f: doc = json.load(f)
                    self.load_document(doc)
                    self.console.append("📂 已恢复自动保存的草稿", "info")
                    return True
                except: pass
        return False
